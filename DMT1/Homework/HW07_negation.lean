...
theorem deMorganDisjunction {P Q : Prop} : ¬(P ∨ Q) ↔ (¬P ∧ ¬Q) :=
  Iff.intro
    ( fun h_not_pvq =>
        And.intro
          (fun p => h_not_pvq (Or.inl p))
          (fun q => h_not_pvq (Or.inr q))
    )
    ( fun h_np_and_nq =>
        fun h_pvq =>
          match h_pvq with
          | Or.inl p => h_np_and_nq.left p
          | Or.inr q => h_np_and_nq.right q
    )